...
The ACK `seq_num` values for `START` and `END` messages should both be set to whatever the `seq_num` values are that were sent by `sender`. For each connection,  `seq_num` starts from 0. 
...
[Survey link](https://docs.google.com/forms/d/e/1FAIpQLSdD65ghmcbNjCUJx-b7nEIf4Gb383zz85FzesPWhwUiOugdcw/viewform?usp=sf_link)
